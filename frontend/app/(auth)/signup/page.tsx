  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.data && res.data.success) {
        alert('Email sent to your mail id. Please verify your email before logging in.');
        router.push('/login');
      } else {
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="pt-6 md:ml-36">
              <button
                type="submit"
                disabled={loading}
                className="w-full bg-pink-600 hover:bg-pink-700 active:scale-[0.98] text-white font-medium py-3 rounded-lg transition-all shadow-sm font-semibold flex items-center justify-center space-x-2 disabled:opacity-70 disabled:cursor-not-allowed"
              >
                {loading ? (
                  <>
